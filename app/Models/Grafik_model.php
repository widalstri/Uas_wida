...
class Grafik_model extends Model
{
	...
	public function grafik_provinsi_json(){ 
		...
		left join tab_provinsi as b on a.provinsi = b.id group by kategori");
	    return $query->getResult();
	}

	public function grafik_tanggalresmi_json(){ 
		$query = $this->db->query("select YEAR (tgl_resmi),count(id) as jumlah from tab_masjid group by tgl_resmi");
	    return $query->getResult();
	}
   
	public function grafik_tanggaldata_json(){ 
		$query = $this->db->query("select YEAR (tanggal_masuk),count(id) as jumlah from tab_masjid group by tanggal_masuk");
	    return $query->getResult();
	}
}